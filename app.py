...
if data_source == 'Тесты':
    ...
    if dataset_type == 'Идеально разделимые':
        X, y = make_blobs(n_samples=1500, centers=[[2, 2], [8, 8]], cluster_std=1.0, random_state=42)
    elif dataset_type == 'Пересекающиеся':
        X, y = make_blobs(n_samples=1500, centers=[[4, 4], [5, 5]], cluster_std=1.5, random_state=42)
